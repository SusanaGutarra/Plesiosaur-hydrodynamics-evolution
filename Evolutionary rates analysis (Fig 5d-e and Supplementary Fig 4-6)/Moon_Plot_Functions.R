...
xAxisPeriods <- function (cex = 1.0) {
  ...
  rect(par()$usr[1], axis.bottom, par()$usr[2], axis.top,
       ...
       # Paleogene
       rect(66, axis.bottom, max(par()$usr[2], 23.03), axis.top,
            col = "white", border = TRUE, xpd = NA)
       text(mean(c(66, max(par()$usr[2], 23.03))),
            mean(c(axis.bottom, axis.top)),
            "PAL",
            adj = c(0.5, 0.5), cex = cex, xpd = NA)
       # Neogene
       rect(23.03, axis.bottom, max(par()$usr[2], 0), axis.top,
            col = "white", border = TRUE, xpd = NA)
       text(mean(c(23.03, max(par()$usr[2], 0))),
            mean(c(axis.bottom, axis.top)),
            "NEO",
            adj = c(0.5, 0.5), cex = cex, xpd = NA)
       # tidy up box around
